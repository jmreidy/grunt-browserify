...
GruntBrowserifyRunner.prototype = _.create(GruntBrowserifyRunner.prototype, {
  run: function (files, destination, options, next) {
    ...
    var bundleComplete = this.onBundleComplete(destination, next);

    doBundle(b, options, bundleComplete);

    if (options.watch) {
      var bundleUpdate = this.onBundleComplete(destination, this.keepAliveFn);
  ...
  onBundleComplete: function (destination, next) {
    ...
    return function (err, src) {
      ...
      self.logger.log.ok('Bundled ' + destination);
      next();
    };
  }
});
...
function doBundle(browserifyInstance, opts, bundleComplete) {
  ...
  browserifyInstance.bundle((opts.browserifyBundleOptions || {}), function (err, src) {
